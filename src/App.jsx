...
function useBidirectionalFilters(store,seller,asinF,masterList){
  return useMemo(()=>{
    // store may be "All", a single name, or "Shop1,Shop2" (multi-select)
    const storeSet = store && store !== 'All'
      ? new Set(store.split(',').map(s=>s.trim()).filter(Boolean))
      : null;
    const ex=excl=>{
      let d=masterList;
      if(excl!=='store' && storeSet) d=d.filter(x=>storeSet.has(x.st));
      if(excl!=='seller' && seller!=='All') d=d.filter(x=>x.sl===seller);
      if(excl!=='asin' && asinF!=='All') d=d.filter(x=>x.a===asinF);
      return d;
    };
    return{
      stores:[...new Set(ex('store').map(x=>x.st))].filter(Boolean).sort(),
      sellers:[...new Set(ex('seller').map(x=>x.sl))].filter(Boolean).sort(),
      asins:[...new Set(ex('asin').map(x=>x.a))].filter(Boolean).sort(),
    };
...
/* ── AsinImg: thumbnail + click-to-zoom preview ── */
function AsinImg({asin,img,size=32,t}){
  ...
  return<>
    <div style={{width:size,height:size,flexShrink:0,cursor:err?'default':'zoom-in',borderRadius:6,overflow:'hidden',background:t.tableBg,display:'flex',alignItems:'center',justifyContent:'center'}}
      onClick={e=>{if(!err){e.stopPropagation();setZoom(true);}}}>
      {err
...
function ExecPage({t,fAsin,fShop,fDaily,em,sd,ed,setSd,setEd,prevEm,prevPeriod,pctChg,mob,onAsinClick,splyEm,dailyLY,shopExt,
  ...
  useEffect(()=>{
    ...
    const h=e=>{
      if(presetRef.current&&presetRef.current.contains(e.target))return;
      if(e.target.closest&&e.target.closest('[data-preset-dropdown]'))return;
      setPresetOpen(false);
    };
    document.addEventListener('mousedown',h);return()=>document.removeEventListener('mousedown',h);
  },[presetOpen]);
    ...
    {(()=>{
      ...
      return<Cd t={t} style={{marginBottom:16}}>
        ...
        <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:10}}>
          ...
          {[{k:'crPct',l:'CR%',c:'#7C3AED'},{k:'tacos',l:'TACoS',c:'#B45309'},{k:'margin',l:'Margin',c:'#059669'},{k:'aov',l:'AOV',c:'#F59E0B'},{k:'roas',l:'ROAS',c:'#DB2777'},{k:'sessions',l:'Sessions',c:'#475569'}].map(li=><button key={li.k} onClick={()=>setTrendLines(p=>({...p,[li.k]:!p[li.k]}))} style={{padding:'4px 10px',borderRadius:8,border:'1px solid '+(trendLines[li.k]?li.c:t.inputBorder),background:trendLines[li.k]?li.c+'18':'transparent',color:trendLines[li.k]?li.c:t.textMuted,fontSize:10,fontWeight:600,cursor:'pointer'}}>{li.l}</button>)}
        ...
        <table style={{width:'100%',borderCollapse:'separate',borderSpacing:0,fontSize:12.5}}>
          ...
          <tbody>{groupedAsins.map((r,i)=>{
            ...
            <td style={{padding:'4px 6px',borderBottom:'1px solid '+t.divider,width:40,height:42,verticalAlign:'middle',lineHeight:0}}>
              {groupBy==='ASIN'&&r.a?<AsinImg asin={r.a} img={r.img||null} size={32} t={t}/>:<div style={{width:32,height:32}}/>}
            </td>
    ...
    <Sec title={"ASIN Stock Detail"+(asinRows.length>0?" ("+asinRows.length+" ASINs)":"")} icon="" t={t} style={{marginTop:14}}>
      <Cd t={t} style={{padding:0,overflow:'hidden'}}>
        ...
        <div style={{overflowX:'auto',maxHeight:480,overflowY:'auto'}}>
          <table style={{width:'100%',borderCollapse:'separate',borderSpacing:0,fontSize:12.5}}>
            <thead style={{position:'sticky',top:0,zIndex:2}}><tr>
              <th style={{padding:'9px 6px',width:40,borderBottom:'2px solid '+t.divider,background:t.tableBg}}/>
              ...
              <th style={{padding:'9px 12px',fontSize:10,fontWeight:700,color:t.textMuted,textTransform:'uppercase',borderBottom:'2px solid '+t.divider,background:t.tableBg,whiteSpace:'nowrap',textAlign:'center'}}>Seller</th>
              <th style={{padding:'9px 12px',fontSize:10,fontWeight:700,color:t.primary,textTransform:'uppercase',borderBottom:'2px solid '+t.divider,background:t.tableBg,whiteSpace:'nowrap',textAlign:'center'}}>Content</th>
              <th style={{padding:'9px 12px',fontSize:10,fontWeight:700,color:t.purple||'#7C3AED',textTransform:'uppercase',borderBottom:'2px solid '+t.divider,background:t.tableBg,whiteSpace:'nowrap',textAlign:'center'}}>Image</th>
            ...
            <tbody>{filteredAsin.length===0?<tr><td colSpan={17} style={{padding:30,textAlign:'center',color:t.textMuted,fontSize:12}}>{asinRows.length===0?'Loading inventory data…':'No results for "'+asinSearch+'"'}</td></tr>:filteredAsin.map((r,i)=>{
              ...
              return<tr key={i} onMouseEnter={e=>e.currentTarget.style.background=t.tableHover} onMouseLeave={e=>e.currentTarget.style.background='transparent'} style={{background:oos?t.redBg+'55':'transparent'}}>
                <td style={{padding:'4px 6px',borderBottom:'1px solid '+t.divider,width:40,height:42,verticalAlign:'middle',lineHeight:0}}>
                  <AsinImg asin={r.asin} img={r.imageUrl||null} size={32} t={t}/>
                </td>
                ...
                <td style={{padding:'6px 12px',borderBottom:'1px solid '+t.divider,verticalAlign:'middle',minWidth:140}}>
                  <div style={{display:'flex',flexDirection:'column',gap:2}}>
                    {[['0–90d',r.age0_90,t.green],['91–180d',r.age91_180,t.orange],['181–365d',r.age181_365,t.orange],['366d+',r.age366plus,t.red]].map(([label,val,col])=>(
                      <div key={label} style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:12,fontSize:11}}>
                        <span style={{color:t.textMuted,whiteSpace:'nowrap'}}>{label}</span>
                        <span style={{fontWeight:val>0?600:400,color:val>0?col:t.textMuted,minWidth:36,textAlign:'right'}}>{N(val)}</span>
                      </div>
                    ))}
                  </div>
                </td>
                {/* Seller */}
                <td style={{padding:'8px 12px',borderBottom:'1px solid '+t.divider,verticalAlign:'middle',textAlign:'center',minWidth:80,fontSize:12,fontWeight:600,color:t.text}}>{r.sellers||<span style={{color:t.textMuted}}>—</span>}</td>
                {/* Content */}
                <td style={{padding:'6px 12px',borderBottom:'1px solid '+t.divider,verticalAlign:'middle',textAlign:'center',minWidth:100}}>
                  {(()=>{const vs=(r.content1||'').split(/[,/;]/).map(s=>s.trim()).filter(Boolean);return vs.length?<div style={{display:'flex',flexWrap:'wrap',gap:3,justifyContent:'center'}}>{vs.map((c,i)=><span key={i} style={{padding:'2px 8px',borderRadius:10,background:t.primaryLight,color:t.primary,fontSize:11,fontWeight:600,whiteSpace:'nowrap'}}>{c}</span>)}</div>:<span style={{color:t.textMuted}}>—</span>;})()}
                </td>
                {/* Image */}
                <td style={{padding:'6px 12px',borderBottom:'1px solid '+t.divider,verticalAlign:'middle',textAlign:'center',minWidth:100}}>
                  {(()=>{const vs=(r.image||'').split(/[,/;]/).map(s=>s.trim()).filter(Boolean);return vs.length?<div style={{display:'flex',flexWrap:'wrap',gap:3,justifyContent:'center'}}>{vs.map((c,i)=><span key={i} style={{padding:'2px 8px',borderRadius:10,background:(t.purple||'#7C3AED')+'22',color:t.purple||'#7C3AED',fontSize:11,fontWeight:600,whiteSpace:'nowrap'}}>{c}</span>)}</div>:<span style={{color:t.textMuted}}>—</span>;})()}
                </td>
          ...
          <tbody>
            {pageRows.map((r,i)=>{
              ...
              return<tr key={i}
                ...
                <td style={{padding:'4px 6px',borderBottom:`1px solid ${t.divider}`,width:40,height:42,verticalAlign:'middle',lineHeight:0}}>
                  <AsinImg asin={r.a} img={asinImgMap[r.a]||null} size={30} t={t}/>
                </td>
          ...
          <tbody>
            {bkRows.map((r,i)=>{
              ...
              return<tr key={i}
                ...
                <td style={{padding:'4px 6px',borderBottom:"1px solid "+t.divider,width:40,height:40,verticalAlign:'middle',lineHeight:0}}>
                  <AsinImg asin={r.a} img={asinImgMap[r.a]||null} size={30} t={t}/>
                </td>
...
function ProdPage({t,isDark,fAsin,fDaily,onAsinClick,sd,ed,store}){
  ...
  const avgBb=useMemo(()=>{const arr=fAsin.filter(a=>a.bb>0);return arr.length?(arr.reduce((s,a)=>s+a.bb,0)/arr.length):0;},[fAsin]);
  const maxSes=useMemo(()=>Math.max(1,...fAsin.map(a=>a.ses)),[fAsin]);
  ...
  const closeDrill=()=>{setSelAsin(null);setDrillData([]);};

  const healthBadge=(cr,bb,margin)=>{
    if(margin>15&&cr>=8&&bb>=90)return{lbl:'Strong',bg:t.greenBg,c:t.green};
    if(margin>0&&(cr>=5||bb>=85))return{lbl:'Fair',bg:t.orangeBg,c:t.orange};
    return{lbl:'Poor',bg:t.redBg,c:t.red};
  };

  const TH={padding:'10px 12px',fontWeight:700,fontSize:11,textTransform:'uppercase',letterSpacing:.5,borderBottom:'2px solid '+t.divider,background:t.tableBg,cursor:'pointer',whiteSpace:'nowrap',userSelect:'none',color:t.textMuted};
  const TH_AMZ={...TH,color:'#185FA5'};
  const TH_SB={...TH,color:t.green};
  const TD={padding:'9px 12px',borderBottom:'1px solid '+t.divider,fontSize:13};
  ...
  return<div>
    ...
    <Sec title="ASIN Table" icon="" t={t} action={
      ...
      <div style={{borderRadius:10,border:'1px solid '+t.cardBorder,background:t.card,overflow:'hidden'}}>
        <div style={{overflowX:'auto',maxHeight:480,overflowY:'auto'}}>
          <table style={{width:'100%',borderCollapse:'separate',borderSpacing:0}}>
            <thead style={{position:'sticky',top:0,zIndex:2}}>
              <tr>
                <th style={{...TH,width:40,padding:'9px 6px',cursor:'default'}}/>
                ...
                <th style={{...TH_SB,textAlign:'right'}} onClick={()=>sortBy('r')}>Revenue{sIco('r')}</th>
                <th style={{...TH_SB,textAlign:'right'}} onClick={()=>sortBy('n')}>Net Profit{sIco('n')}</th>
                <th style={{...TH_SB,textAlign:'right'}} onClick={()=>sortBy('m')}>Margin{sIco('m')}</th>
                <th style={{...TH_SB,textAlign:'right'}} onClick={()=>sortBy('u')}>Units{sIco('u')}</th>
                <th style={{...TH_SB,textAlign:'right'}} onClick={()=>sortBy('ac')}>ACoS{sIco('ac')}</th>
                <th style={{...TH_AMZ,textAlign:'right'}} onClick={()=>sortBy('ses')}>Sessions{sIco('ses')}</th>
                <th style={{...TH_AMZ,textAlign:'right'}} onClick={()=>sortBy('cr')}>Conv. Rate{sIco('cr')}</th>
                <th style={{...TH_AMZ,textAlign:'right'}} onClick={()=>sortBy('bb')}>Buy Box%{sIco('bb')}</th>
                <th style={{...TH,textAlign:'center',cursor:'default'}}>Health</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((r,i)=>{
                ...
                <tr onClick={()=>isSel?closeDrill():selectAsin(r)}
                  ...
                  <td style={{...TD,padding:'4px 6px',width:40,height:40,verticalAlign:'middle',lineHeight:0,borderBottom:isSel?'none':TD.borderBottom}}>
                    <AsinImg asin={r.a} img={r.img||null} size={30} t={t}/>
                  </td>
...
/* ═══════════ TEAM ═══════════ */
function TeamPage({t,fSeller,fDaily,asinPlanBkData,onAsinClick,sd,ed,store,seller,asinImgMap={}}){
  ...
  return<div>
    ...
    {asinData.length>0&&<Sec title="ASIN Detail by Seller (A / P / Gap)" icon="" t={t}><div style={{borderRadius:12,border:"1px solid "+t.cardBorder,background:t.card,overflow:"hidden"}}><div style={{overflowX:"auto",maxHeight:520,overflowY:"auto"}}><table style={{width:"100%",borderCollapse:"separate",borderSpacing:0,fontSize:12.5}}><thead style={{position:"sticky",top:0,zIndex:2}}><tr><th style={{padding:"9px 6px",width:40,borderBottom:"2px solid "+t.divider,background:t.tableBg}}/>{THDASIN.map((h,i)=><th key={i} style={{padding:"11px 14px",textAlign:i<=2?"left":"right",color:h==="STOCK VALUE"?t.orange:h.includes("GP")?t.primary:t.textMuted,fontWeight:700,fontSize:10.5,textTransform:"uppercase",letterSpacing:.5,borderBottom:"2px solid "+t.divider,background:h==="STOCK VALUE"?t.tableBg:h.includes("GP")?t.primaryLight:t.tableBg,whiteSpace:"nowrap",minWidth:i<=2?70:100}}>{h}{h==="STOCK VALUE"&&<Tip text={TIPS.stockValue} t={t}/>}</th>)}</tr></thead><tbody>{asinData.map((r,i)=><tr key={i} onMouseEnter={e=>e.currentTarget.style.background=t.tableHover} onMouseLeave={e=>e.currentTarget.style.background="transparent"} style={sellerBreaks.has(i)?{borderTop:"2px solid "+t.divider}:{transition:"background .1s"}}><td style={{padding:"4px 6px",width:40,height:42,verticalAlign:"middle",lineHeight:0,borderBottom:"1px solid "+t.divider}}><AsinImg asin={r.a} img={asinImgMap[r.a]||null} size={30} t={t}/></td><td style={{padding:"11px 14px",fontSize:13,fontWeight:600,letterSpacing:.3,borderBottom:"1px solid "+t.divider,color:t.textSec}}><AsinLink asin={r.a} onClick={onAsinClick||(()=>{})} t={t}/></td><td style={{padding:"11px 14px",fontWeight:600,borderBottom:"1px solid "+t.divider,fontSize:11.5,color:t.primary}}>{r.sl||"—"}</td><td style={{padding:"11px 14px",fontWeight:700,borderBottom:"1px solid "+t.divider}}>{r.br}</td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider,background:t.primaryGhost}}><APG actual={r.ga} plan={r.gp} t={t}/></td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider}}><APG actual={r.ra} plan={r.rp} t={t}/></td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider}}><APG actual={r.aa} plan={r.ap} t={t} reverse/></td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider}}><APG actual={r.ua} plan={r.up} t={t} isMoney={false}/></td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider}}><StockVal sv={r.sv} gp={r.ga} t={t}/></td></tr>)}</tbody></table></div><div style={{padding:"8px 14px",fontSize:10.5,color:t.textMuted,borderTop:"1px solid "+t.divider,background:t.card}}>{asinData.length} ASINs · Grouped by seller · Stock Value = current snapshot</div></div></Sec>}
...
function ProductCRPage({t,sd,ed,store}){
  ...
  return<div>
    ...
    {!loading&&period==='weekly'&&filtered.length>0&&periodLabels.length>0&&(()=>{
      ...
      const INFO_COLS = hasAnyContent23 ? 12 : 10;
...
/* ═══════════ ANALYTICS PAGE ═══════════ */
function AnalyticsPage({t,fDaily,fShopData,fSeller,fAsin,em,monthPlanData,monthlyLY,sd,ed}){
...
function Dashboard({authUser,onLogout}){
  ...
  const[monthlyLYState,setMonthlyLYState]=useState([]);
  ...
  pgRef.current=pg;
  // Cache: tracks which (page, filterKey) combos have been fetched
  const fetchCacheRef=useRef({key:null,pages:new Set()});

  // Fetch trigger fires on filter change OR page switch
  useEffect(()=>{
    if(!live||dbConnecting)return;
    const timer=setTimeout(()=>setFetchTrigger(t=>t+1),300);
    return()=>clearTimeout(timer);
  },[sd,ed,store,seller,asinF,productType,niche,pg,live,dbConnecting]);
  ...
  useEffect(()=>{
    ...
    // Cache key from filters only (page tracked separately as Set entries)
    const cacheKey=JSON.stringify({_sd,_ed,_st,_sl,_af,_pt,_ni});
    if(fetchCacheRef.current.key!==cacheKey){
      // Filter changed → invalidate all cached pages
    ...
    (async()=>{try{
      // Batch 1: exec/summary + exec/daily (only for exec or daily ops)
      if(needDaily && !alreadyFetched.has('exec:'+cacheKey)){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        const[planRes,actualsRes,lyRes]=await Promise.all([
          ...
          api("exec/monthly-ly",{store:_st,seller:_sl,asin:_af},30000).catch(e=>{console.error("monthly-ly ERROR:",e.message);return[];})
        ]);
        ...
        console.log("monthly-ly:",lyRes?.length,"rows");
        if(cancelled)return;
        setMonthlyLYState(Array.isArray(lyRes)?lyRes:[]);
