...
const TH={
  ...
  dark:{bg:"#0C0E16",card:"#161825",cardBorder:"#2A2E42",sidebar:"#111320",sidebarBorder:"#252840",sidebarActive:"#1E2348",topbar:"#111320",text:"#F1F3F9",textSec:"#B0B5CC",textMuted:"#727A96",primary:"#8B9EF0",primaryLight:"#1E2348",primaryGhost:"#171A30",green:"#4AE09A",greenBg:"#0F2E20",red:"#FF7A6A",redBg:"#2E1616",orange:"#FFC05C",orangeBg:"#2E2412",blue:"#6CB4FF",purple:"#B494FF",chartGrid:"#2A2E42",inputBg:"#1A1D2E",inputBorder:"#353952",tableBg:"#13152200",tableHover:"#1E2348",divider:"#252840",kpiIcon:"#1E2348",shadow:"rgba(0,0,0,0.5)"},
};
...
const Sec=({title,icon,t,action,children})=><div style={{marginTop:28}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}><div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:17}}>{icon}</span><span style={{fontSize:16,fontWeight:700,color:t.text,letterSpacing:-.2}}>{title}</span></div>{action}</div>{children}</div>;
...
export default function App(){
  ...
  return<div style={{display:"flex",flexDirection:mob?"column":"row",height:"100vh",background:t.bg,fontFamily:"'DM Sans',system-ui,-apple-system,sans-serif",color:t.text,overflow:"hidden",transition:"background .3s"}}>
    ...
    {!mob&&<div style={{width:(tab||!sb)?56:220,background:t.sidebar,borderRight:"1px solid "+t.sidebarBorder,display:"flex",flexDirection:"column",transition:"width .2s",flexShrink:0,overflow:"hidden"}}>
      ...
      <div style={{flex:1,padding:6,overflowY:"auto"}}>{NAV.map(n=><button key={n.id} onClick={()=>setPg(n.id)} style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:(!tab&&sb)?"12px 14px":"12px 0",borderRadius:10,border:"none",cursor:"pointer",marginBottom:3,background:pg===n.id?t.sidebarActive:"transparent",color:pg===n.id?t.primary:t.textSec,justifyContent:(!tab&&sb)?"flex-start":"center",fontSize:13.5,transition:"all .15s ease"}} onMouseEnter={e=>{if(pg!==n.id)e.currentTarget.style.background=t.tableHover}} onMouseLeave={e=>{if(pg!==n.id)e.currentTarget.style.background="transparent"}}><span style={{fontSize:17,flexShrink:0}}>{n.i}</span>{!tab&&sb&&<span style={{fontWeight:pg===n.id?700:500,whiteSpace:"nowrap",letterSpacing:-.1}}>{n.l}</span>}</button>)}</div>
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",paddingBottom:mob?56:0}}>
      ...
      <div style={{background:t.topbar,borderBottom:"1px solid "+t.cardBorder,padding:mob?"10px 14px":"14px 24px",display:"flex",flexDirection:"column",gap:10,flexShrink:0}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>{mob&&<button onClick={()=>setMobileFilters(!mobileFilters)} style={{background:t.primaryLight,border:"1px solid "+t.primary+"33",borderRadius:10,padding:"7px 12px",cursor:"pointer",fontSize:12,color:t.primary,fontWeight:700}}>☰</button>}<span style={{fontSize:mob?17:20,fontWeight:800,color:t.text,letterSpacing:-.4}}>{cn?.i} {cn?.l}</span></div>
