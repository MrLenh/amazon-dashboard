...
/* ═══════════ SHARED COMPONENTS ═══════════ */
function Tip({text,t}){const[s,setS]=useState(false);return<span style={{position:"relative",display:"inline-flex",cursor:"help"}} onMouseEnter={()=>setS(true)} onMouseLeave={()=>setS(false)}><span style={{fontSize:10,color:t.textMuted,marginLeft:3}}>ⓘ</span>{s&&<div style={{position:"absolute",top:"calc(100% + 6px)",left:"50%",transform:"translateX(-50%)",background:t.text,color:t.bg,padding:"6px 10px",borderRadius:6,fontSize:10,whiteSpace:"normal",textTransform:"none",zIndex:999,boxShadow:"0 4px 12px rgba(0,0,0,.2)",fontWeight:500,maxWidth:280,lineHeight:1.4}}>{text}</div>}</span>}
...
function ExecPage({t,fAsin,fShop,fDaily,em,sd,ed,prevEm,pctChg,mob}){
  ...
  return<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"repeat(4,1fr)",gap:12,marginTop:14,marginBottom:14}}>
      ...
      <KpiCard title="Avg Order Value" value={em.orders>0?$2(em.sales/em.orders):"—"} icon="🛍" t={t} tip={TIPS.aov}/>
      <KpiCard title="Units per Order" value={em.orders>0?(em.units/em.orders).toFixed(1):"—"} icon="📦" t={t} tip={TIPS.upo}/>
    </div>
...
function InvPage({t,mob,invData,invShop,invTrend}){
  ...
  return<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(148px,1fr))",gap:12,marginBottom:16}}>
      <KpiCard title="FBA Stock" value={N(d.fbaStock||0)} icon="📦" t={t} tip={TIPS.fbaStock}/><KpiCard title="Available" value={N(d.availableInv||0)} icon="🗃" t={t} tip={TIPS.invAvail}/><KpiCard title="Reserved" value={N(d.reserved||0)} icon="🔒" t={t} tip={TIPS.invReserved}/><KpiCard title="Critical SKUs" value={N(d.criticalSkus||0)} icon="🚨" t={t} tip={TIPS.invCritical}/><KpiCard title="Inbound" value={N(d.inbound||0)} icon="📥" t={t} tip={TIPS.invInbound}/><KpiCard title="Avg Days Supply" value={Math.round(d.avgDaysOfSupply||0)} icon="📈" t={t} tip={TIPS.invDaysSupply}/>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"1fr 1fr",gap:14,marginTop:14}}>
      ...
      <Sec title="FBA Stock by Shop" icon="📦" t={t}><Cd t={t}><ResponsiveContainer width="100%" height={Math.max(220,invShop.length*28)}><BarChart data={[...invShop].sort((a,b)=>b.fba-a.fba)} layout="vertical" margin={{left:100,right:20}} barSize={14}><CartesianGrid strokeDasharray="3 3" stroke={t.chartGrid}/><XAxis type="number" tick={{fill:t.textMuted,fontSize:9}} tickFormatter={N}/><YAxis type="category" dataKey="s" tick={{fill:t.textSec,fontSize:9}} width={95}/><Tooltip content={<CT t={t}/>}/><Bar dataKey="fba" name="FBA Stock" fill={t.primary} radius={[0,4,4,0]}/></BarChart></ResponsiveContainer></Cd></Sec>
    </div>
