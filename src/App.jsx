...
const demoDaily=(()=>{const jR=[18200,19500,17800,16900,15200,14100,12800,18900,20100,21500,19800,20400,21200,22100,20800,19600,21800,23200,22500,21100,22800,24100,23500,22200,24800,25500,23800,26100,27200,25900,28500];const jN=[-1200,-800,-1500,-2100,-3400,-38000,-5200,-1800,400,1200,-200,600,900,1500,300,-500,1100,2000,1600,800,1800,2500,2100,1400,2900,3200,2200,3600,4100,3400,4800];const jU=[980,1050,920,870,780,710,650,990,1080,1150,1060,1090,1130,1180,1110,1050,1170,1250,1210,1130,1220,1290,1260,1190,1330,1370,1280,1400,1460,1390,1530];const fR=[27800,26500,25200,24800,23100,21900,22500,25800,27100,28400,26900,27500,28200,29100,27800,26200,28500,29800,28900,27200,29500,30200,29100,28500,31000,30500,29800];const fN=[4200,3800,3100,2800,1900,1200,1600,3500,4300,5100,4200,4600,5000,5800,4500,3800,5200,5900,5400,4100,5600,6200,5500,4800,6500,6100,5700];const fU=[1490,1420,1350,1330,1240,1170,1210,1380,1450,1520,1440,1470,1510,1560,1490,1400,1530,1600,1550,1460,1580,1620,1560,1530,1660,1640,1600];const d=[];for(let i=0;i<31;i++){const p=String(i+1).padStart(2,"0");d.push({date:"2026-01-"+p,label:"Jan "+(i+1),revenue:jR[i],netProfit:jN[i],units:jU[i]})}for(let i=0;i<27;i++){const p=String(i+1).padStart(2,"0");d.push({date:"2026-02-"+p,label:"Feb "+(i+1),revenue:fR[i],netProfit:fN[i],units:fU[i]})}return d})();
...
const monthPlan=[
  ...
  {m:"Feb",gpa:5200,gpp:19000,ra:720000,rp:620000,aa:168000,ap:160000,ua:39500,up:33000,sa:510000,sp:360000,ia:33500000,ip:30000000,cra:7.75,crp:9.17,cta:1.28,ctp:1.20},
...
const salesVel=[{d:"Jan 18",v:1049,ma:980},{d:"Jan 20",v:3834,ma:1350},{d:"Jan 23",v:663,ma:1100},{d:"Jan 27",v:1483,ma:1100},{d:"Feb 01",v:945,ma:1180},{d:"Feb 05",v:2108,ma:1350},{d:"Feb 08",v:821,ma:1200},{d:"Feb 12",v:1650,ma:1280},{d:"Feb 16",v:1920,ma:1400},{d:"Feb 20",v:1340,ma:1350},{d:"Feb 24",v:1780,ma:1420}];
...
function genAlerts(fAsin,t,extra){
  ...
  if(extra)extra.forEach(e=>alerts.push(e));
...
function genSellerAlerts(sellers){const a=[];const top=[...sellers].sort((x,y)=>y.n-x.n)[0];const low=[...sellers].sort((x,y)=>x.m-y.m)[0];const hiU=sellers.filter(s=>s.u90>5000);if(top)a.push({s:"i",t:`Top seller: ${top.sl} — ${$(top.n)} profit (${top.m.toFixed(1)}% margin)`});if(low&&low.m<5)a.push({s:"w",t:`Lowest margin: ${low.sl} at ${low.m.toFixed(1)}% — needs attention`});if(hiU.length)a.push({s:"c",t:`${hiU.length} sellers with >5K units aged >90d: ${hiU.map(s=>s.sl).join(", ")}`});return a}
  ...
  return<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"1fr 1fr",gap:14,marginBottom:16}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"1fr 1fr",gap:10}}>
        <KpiCard title="Revenue" value={$(em.sales)} change={ch("sales")} icon="💰" t={t} tip={TIPS.sales}/><KpiCard title="Net Profit" value={$(em.netProfit)} change={ch("netProfit")} icon="📈" t={t} tip={TIPS.netProfit}/>
        <KpiCard title="Margin" value={(em.margin||0).toFixed(2)+"%"} change={prevEm?em.margin-prevEm.margin:undefined} icon="🎯" t={t} tip={TIPS.margin}/><KpiCard title="Orders" value={N(em.orders)} change={ch("orders")} icon="🛒" t={t}/>
    ...
    <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"1.4fr .6fr",gap:14,marginTop:16}}>
      <Sec title="Revenue & NP by Shop" icon="🏪" t={t}><Cd t={t}><ResponsiveContainer width="100%" height={Math.max(200,fShop.length*38)}><BarChart data={fShop} layout="vertical" margin={{left:90,right:10}}><CartesianGrid strokeDasharray="3 3" stroke={t.chartGrid} horizontal={false}/><XAxis type="number" tick={{fill:t.textMuted,fontSize:10}} tickFormatter={v=>$s(v)}/><YAxis type="category" dataKey="s" tick={{fill:t.textSec,fontSize:10}} width={85}/><Tooltip content={<CT t={t}/>}/><Legend wrapperStyle={{fontSize:10}}/><Bar dataKey="r" name="Revenue" fill={t.primary} radius={[0,4,4,0]} barSize={14}/><Bar dataKey="n" name="Net Profit" radius={[0,4,4,0]} barSize={14}>{fShop.map((e,i)=><Cell key={i} fill={e.n>=0?t.green:t.red}/>)}</Bar></BarChart></ResponsiveContainer></Cd></Sec>
...
function ShopPage({t,fShopData,fDaily}){
  return<div>
    ...
    <Sec title="Revenue Trend" icon="📈" t={t}><TrendChart data={fDaily} t={t} height={220} showNP={false}/></Sec>
    ...
    <div style={{marginTop:14}}><Alerts t={t} alerts={genShopAlerts(fShopData)}/></div>
  </div>;
}
...
function TeamPage({t,fSeller,fDaily}){
  return<div>
    ...
    <div style={{marginTop:14}}><Alerts t={t} alerts={genSellerAlerts(fSeller)}/></div>
  </div>;
}
...
function OpsPage({t,fDaily,fShopData}){
  return<div>
    ...
    <div style={{marginTop:14}}><Alerts t={t} alerts={genOpsAlerts(fDaily,fShopData)}/></div>
  </div>;
}

/* ═══════════ AI INSIGHT ═══════════ */
function AiInsight({t,context,live}){const[open,setOpen]=useState(false);const[loading,setLoading]=useState(false);const[insight,setInsight]=useState("");const[question,setQuestion]=useState("");const run=async()=>{setLoading(true);setInsight("");if(live){try{const data=await apiPost("ai/insight",{context,question:question||undefined});setInsight(data.insight||"No insight")}catch(e){setInsight("Error: "+e.message)}}else{await new Promise(r=>setTimeout(r,800));const m=context.em;const top=context.fAsin?.[0];let txt="📊 Dashboard Analysis\n\n";txt+=`Revenue: ${$2(m.sales)} | Net Profit: ${$2(m.netProfit)} | Margin: ${(m.margin||0).toFixed(1)}%\n\n`;if(m.margin<0)txt+=`⚠️ Negative margin detected. Key cost drivers:\n• Ad Spend: ${$2(Math.abs(m.advCost))} (ACoS ${(m.realAcos||0).toFixed(1)}%)\n• Amazon Fees: ${$2(Math.abs(m.amazonFees))}\n• COGS: ${$2(Math.abs(m.cogs))}\n\n`;else txt+=`✅ Positive margin. Ad efficiency at ${(m.realAcos||0).toFixed(1)}% ACoS.\n\n`;if(top)txt+=`🏆 Top ASIN: ${top.a} (${top.b}) — ${$(top.r)} revenue, ${top.m.toFixed(1)}% margin\n\n`;txt+=`💡 Recommendations:\n• ${m.realAcos>30?"Reduce ACoS — lower bids on underperforming keywords":"Maintain ad strategy — ACoS is healthy"}\n• ${m.pctRefunds>10?"High refund rate ("+m.pctRefunds.toFixed(1)+"%) — investigate quality":"Refund rate within normal range"}\n• Focus on top-margin ASINs for scaling`;if(question)txt+=`\n\n🔍 Re: "${question}"\nConnect your database for real-time AI insights.`;setInsight(txt)}setLoading(false)};if(!open)return<button onClick={()=>setOpen(true)} style={{position:"fixed",bottom:20,right:20,zIndex:999,background:"linear-gradient(135deg,#3B4A8A,#6B7FD7)",color:"#fff",border:"none",borderRadius:14,padding:"12px 18px",cursor:"pointer",boxShadow:"0 4px 20px rgba(59,74,138,.3)",fontSize:13,fontWeight:700}}>🤖 AI Insight</button>;return<div style={{position:"fixed",bottom:20,right:20,zIndex:999,width:420,maxHeight:"70vh",background:t.card,borderRadius:14,border:"1px solid "+t.cardBorder,boxShadow:"0 12px 40px "+t.shadow,display:"flex",flexDirection:"column",overflow:"hidden"}}><div style={{padding:"12px 16px",borderBottom:"1px solid "+t.divider,display:"flex",justifyContent:"space-between",background:`linear-gradient(135deg,${t.primary},#5A6BC5)`}}><span style={{fontSize:13,fontWeight:700,color:"#fff"}}>🤖 AI Insight</span><button onClick={()=>setOpen(false)} style={{background:"rgba(255,255,255,.2)",border:"none",borderRadius:6,color:"#fff",cursor:"pointer",padding:"4px 8px",fontSize:12}}>✕</button></div><div style={{padding:12,borderBottom:"1px solid "+t.divider}}><input value={question} onChange={e=>setQuestion(e.target.value)} placeholder="Ask a question or leave blank for overview..." style={{width:"100%",padding:"8px 12px",borderRadius:8,border:"1px solid "+t.inputBorder,background:t.inputBg,color:t.text,fontSize:12,boxSizing:"border-box"}} onKeyDown={e=>e.key==="Enter"&&run()}/><button onClick={run} disabled={loading} style={{marginTop:8,width:"100%",padding:"8px",borderRadius:8,border:"none",background:loading?t.textMuted:t.primary,color:"#fff",cursor:loading?"wait":"pointer",fontSize:12,fontWeight:700}}>{loading?"⏳ Analyzing...":"Analyze"}</button></div><div style={{flex:1,overflow:"auto",padding:14}}>{insight?<div style={{fontSize:12,color:t.textSec,lineHeight:1.7,whiteSpace:"pre-wrap"}}>{insight}</div>:<div style={{fontSize:11,color:t.textMuted,textAlign:"center",padding:20}}>Click Analyze for AI-powered insights.</div>}</div></div>}
  ...
  return<div style={{display:"flex",flexDirection:mob?"column":"row",height:"100vh",background:t.bg,fontFamily:"'DM Sans',system-ui,-apple-system,sans-serif",color:t.text,overflow:"hidden",transition:"background .3s"}}>
    ...
    {!mob&&<div style={{width:(tab||!sb)?56:220,background:t.sidebar,borderRight:"1px solid "+t.sidebarBorder,display:"flex",flexDirection:"column",transition:"width .2s",flexShrink:0,overflow:"hidden"}}>
      <div style={{padding:"14px 14px 10px",display:"flex",alignItems:"center",gap:8,borderBottom:"1px solid "+t.sidebarBorder,minHeight:50}}>
        ...
        {!tab&&sb&&<div><div style={{fontSize:14,fontWeight:800,color:t.text,lineHeight:1.1}}>Amazon</div><div style={{fontSize:8,color:t.textMuted,letterSpacing:1.5,fontWeight:700,textTransform:"uppercase"}}>Dashboard</div></div>}
      </div>
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",paddingBottom:mob?56:0}}>
      {/* Topbar */}
      <div style={{padding:mob?"8px 12px":"10px 20px",borderBottom:"1px solid "+t.sidebarBorder,display:"flex",alignItems:"center",justifyContent:"space-between",background:t.topbar,flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:mob?14:16}}>{cn?.i}</span><h1 style={{fontSize:mob?13:16,fontWeight:800,color:t.text,margin:0}}>{cn?.l}</h1></div>
        <div style={{display:"flex",alignItems:"center",gap:mob?4:8}}>
          {mob&&showDate&&<button onClick={()=>setMobileFilters(!mobileFilters)} style={{background:t.inputBg,border:"1px solid "+t.inputBorder,borderRadius:8,padding:"5px 8px",cursor:"pointer",color:t.textSec,fontSize:10,fontWeight:600}}>🔽 Filters</button>}
          <div style={{display:"flex",alignItems:"center",gap:4,padding:"4px 10px",borderRadius:8,background:live?t.greenBg:t.orangeBg,fontSize:10,fontWeight:600,color:live?t.green:t.orange}}><div style={{width:6,height:6,borderRadius:3,background:live?t.green:t.orange}}/>{mob?"":(live?"Live DB":"Demo")}</div>
          <button onClick={()=>setDark(!isDark)} style={{background:t.inputBg,border:"1px solid "+t.inputBorder,borderRadius:8,padding:"6px 10px",cursor:"pointer",color:t.textSec,fontSize:11,fontWeight:600}}>{isDark?"☀️":"🌙"}{!mob&&(isDark?" Light":" Dark")}</button>
        </div>
      </div>
      {/* Filter bar — collapsible on mobile */}
      {(!mob||mobileFilters)&&<div style={{padding:mob?"8px 12px":"8px 20px",borderBottom:"1px solid "+t.sidebarBorder,background:t.topbar,flexShrink:0,display:"flex",alignItems:mob?"stretch":"center",gap:8,flexWrap:"wrap",flexDirection:mob?"column":"row"}}>
        {showDate&&<div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}><DateInput label="Start" value={sd} onChange={v=>{setSd(v);setActivePeriod(null)}} t={t}/><DateInput label="End" value={ed} onChange={v=>{setEd(v);setActivePeriod(null)}} t={t}/><ClearBtn onClick={clearDates} t={t}/></div>}
        {showPeriod&&<PeriodBtns onSelect={(s,e,l)=>{setSd(s);setEd(e);setActivePeriod(l);if(mob)setMobileFilters(false)}} active={activePeriod} t={t}/>}
        ...
        <div style={{marginLeft:mob?0:"auto",display:"flex",gap:6,flexWrap:"wrap"}}>
    ...
    {/* MOBILE BOTTOM NAV */}
    {mob&&<div style={{position:"fixed",bottom:0,left:0,right:0,background:t.sidebar,borderTop:"1px solid "+t.sidebarBorder,display:"flex",justifyContent:"space-around",padding:"6px 0",zIndex:998}}>
