...
function AsinSel({value,onChange,options,label,t}){
  ...
  return<div ref={ref} style={{position:"relative",display:"inline-block"}}><button onClick={()=>{setOpen(!open);setQ("")}} style={{background:t.card,color:value==="All"?t.textMuted:t.primary,border:"1px solid "+(value==="All"?t.inputBorder:t.primary),borderRadius:10,padding:"7px 14px",fontSize:12,fontWeight:value==="All"?500:700,cursor:"pointer",minWidth:110,textAlign:"left",transition:"all .15s",letterSpacing:value==="All"?0:-.3}}>{value==="All"?label:value} <span style={{fontSize:9,opacity:.6}}>▾</span></button>
    {open&&<div style={{position:"absolute",top:"calc(100% + 4px)",left:0,zIndex:999,background:t.card,border:"1px solid "+t.cardBorder,borderRadius:12,boxShadow:"0 8px 32px "+t.shadow,minWidth:220,maxHeight:360,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      <div style={{padding:"10px 10px 8px",borderBottom:"1px solid "+t.divider}}><input value={q} onChange={e=>setQ(e.target.value)} placeholder="🔍 Search ASIN..." autoFocus style={{width:"100%",padding:"8px 12px",border:"1px solid "+t.inputBorder,borderRadius:8,fontSize:12.5,background:t.inputBg,color:t.text,outline:"none",boxSizing:"border-box",transition:"border .15s"}} onFocus={e=>e.target.style.borderColor=t.primary} onBlur={e=>e.target.style.borderColor=t.inputBorder}/></div>
      <div style={{overflowY:"auto",flex:1,padding:"4px 0"}}>
        <div onClick={()=>{onChange("All");setOpen(false)}} onMouseEnter={()=>setHov("all")} onMouseLeave={()=>setHov(null)} style={{padding:"9px 14px",fontSize:12.5,cursor:"pointer",fontWeight:value==="All"?700:500,color:value==="All"?t.primary:t.text,background:value==="All"?t.primaryLight:hov==="all"?t.tableHover:"transparent",transition:"background .1s",borderRadius:6,margin:"0 4px"}}>{label}</div>
        {filtered.length>0&&<div style={{padding:"4px 14px 2px",fontSize:9.5,color:t.textMuted,fontWeight:600,letterSpacing:1,textTransform:"uppercase"}}>{filtered.length} ASINs</div>}
        {filtered.map(o=><div key={o} onClick={()=>{onChange(o);setOpen(false)}} onMouseEnter={()=>setHov(o)} onMouseLeave={()=>setHov(null)} style={{padding:"8px 14px",fontSize:12,letterSpacing:.3,fontWeight:value===o?700:500,color:value===o?t.primary:t.textSec,background:value===o?t.primaryLight:hov===o?t.tableHover:"transparent",cursor:"pointer",transition:"background .1s",borderRadius:6,margin:"0 4px"}}>{o}</div>)}
        {filtered.length===0&&<div style={{padding:"16px 14px",fontSize:12,color:t.textMuted,textAlign:"center"}}>No ASINs found</div>}
      </div>
      {options.length>10&&<div style={{padding:"6px 14px",borderTop:"1px solid "+t.divider,fontSize:10,color:t.textMuted,textAlign:"center",fontWeight:500}}>{options.length} total</div>}
    </div>}
  </div>;
}

function KpiCard({title,value,change,icon,t,tip}){return<div style={{background:t.card,borderRadius:14,padding:"18px 20px",border:"1px solid "+t.cardBorder,overflow:"visible",transition:"all .2s ease"}} onMouseEnter={e=>{e.currentTarget.style.boxShadow="0 8px 28px "+t.shadow;e.currentTarget.style.transform="translateY(-1px)";}} onMouseLeave={e=>{e.currentTarget.style.boxShadow="none";e.currentTarget.style.transform="";}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}><div style={{flex:1,overflow:"visible"}}><div style={{fontSize:10.5,color:t.textMuted,textTransform:"uppercase",letterSpacing:1,fontWeight:700,marginBottom:10}}>{title}{tip&&<Tip text={tip} t={t}/>}</div><div style={{fontSize:22,fontWeight:800,color:t.text,letterSpacing:-.5}}>{value}</div>{change!==undefined&&change!==null&&<div style={{display:"flex",alignItems:"center",gap:4,marginTop:8}}><span style={{fontSize:11,fontWeight:600,color:change>=0?t.green:t.red,background:change>=0?t.greenBg:t.redBg,padding:"3px 10px",borderRadius:10}}>{change>=0?"↑":"↓"} {Math.abs(change).toFixed(1)}%</span><span style={{fontSize:9,color:t.textMuted}}>vs prev</span></div>}</div><div style={{width:36,height:36,borderRadius:10,background:t.kpiIcon,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>{icon}</div></div></div>}

function PlanKpi({title,actual,plan,t,highlight,tip,fmt}){const isN=typeof actual==="number"&&typeof plan==="number";const gap=isN?actual-plan:null;const gc=gap!=null?(gap>=0?t.green:t.red):t.textMuted;const F=fmt||$;return<div style={{background:highlight?t.primaryLight:t.card,borderRadius:14,padding:"18px 20px",border:highlight?"2px solid "+t.primary:"1px solid "+t.cardBorder,overflow:"visible",transition:"all .2s ease"}} onMouseEnter={e=>{e.currentTarget.style.boxShadow="0 8px 28px "+t.shadow;}} onMouseLeave={e=>{e.currentTarget.style.boxShadow="none";}}><div style={{fontSize:10.5,color:highlight?t.primary:t.textMuted,textTransform:"uppercase",letterSpacing:1,fontWeight:700,marginBottom:12}}>{highlight?"⭐ ":""}{title}{tip&&<Tip text={tip} t={t}/>}</div><div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:4}}><span style={{fontSize:11,color:t.textMuted,fontWeight:500}}>Actual</span><span style={{fontSize:highlight?24:20,fontWeight:800,color:highlight?t.primary:t.text,letterSpacing:-.3}}>{isN?F(actual):actual}</span></div><div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline"}}><span style={{fontSize:11,color:t.textMuted,fontWeight:500}}>Plan</span><span style={{fontSize:13,fontWeight:600,color:t.textSec}}>{isN?F(plan):plan}</span></div><div style={{marginTop:12,padding:"8px 12px",borderRadius:8,background:t.primaryGhost,display:"flex",justifyContent:"space-between"}}><span style={{fontSize:10.5,color:t.textMuted,fontWeight:500}}>Gap</span><span style={{fontSize:13,fontWeight:700,color:gc}}>{gap!=null?F(gap):"—"}</span></div></div>}

const Sec=({title,icon,t,action,children})=><div style={{marginTop:24}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}><div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:15}}>{icon}</span><span style={{fontSize:14,fontWeight:700,color:t.text,letterSpacing:-.2}}>{title}</span></div>{action}</div>{children}</div>;
const Cd=({children,t,style:s})=><div style={{background:t.card,borderRadius:14,padding:18,border:"1px solid "+t.cardBorder,...s}}>{children}</div>;
...
function TeamPage({t,fSeller,fDaily,asinPlanBkData}){
  ...
  const asinData=useMemo(()=>{
    ...
    if(teamMonth!=="All"){
      ...
      filtered=raw.map(r=>{
        const md=r.mData?.[mi]||r.mData?.[String(mi)]||{};
        return{...r,ga:md.ga||0,gp:md.gp||0,na:md.na||0,np:md.np||0,ra:md.ra||0,rp:md.rp||0,aa:md.aa||0,ap:md.ap||0,ua:md.ua||0,up:md.up||0,sa:md.sa||0,sp:md.sp||0,ia:md.ia||0,ip:md.ip||0,cra:md.cra||0,crp:md.crp||0,cta:md.cta||0,ctp:md.ctp||0};
      }).filter(r=>Math.abs(r.ga||0)+Math.abs(r.gp||0)+Math.abs(r.ra||0)+Math.abs(r.rp||0)+Math.abs(r.aa||0)+Math.abs(r.ap||0)+Math.abs(r.ua||0)+Math.abs(r.up||0)+Math.abs(r.sa||0)+Math.abs(r.sp||0)>0);
    }
  ...
  const sellerSummary=useMemo(()=>{
    ...
    asinData.forEach(r=>{
      ...
      if(!map[sl])map[sl]={sl,ga:0,gp:0,ra:0,rp:0,aa:0,ap:0,ua:0,up:0,cnt:0};
      const s=map[sl];s.ga+=r.ga||0;s.gp+=r.gp||0;s.ra+=r.ra||0;s.rp+=r.rp||0;s.aa+=r.aa||0;s.ap+=r.ap||0;s.ua+=r.ua||0;s.up+=r.up||0;s.cnt++;
    });
    return Object.values(map).map(s=>({...s,margin:sellerMargins[s.sl]?.margin||0})).sort((a,b)=>(b.ga||0)-(a.ga||0));
  },[asinData,sellerMargins]);
  const THDSL=["Seller","⭐ GP","REVENUE","ADS","UNITS","MARGIN","ASINs"];
  const THDASIN=["ASIN","Seller","Brand","⭐ GP","REVENUE","ADS","UNITS"];
  ...
  return<div>
    ...
    <Sec title="Revenue Trend" icon="📈" t={t}><TrendChart data={fDaily} t={t} keys={[{dk:"revenue",n:"Revenue",c:t.primary,g:"url(#gRv)"}]}/></Sec>
    <Sec title="Seller Performance (A / P / Gap)" icon="🎯" t={t} action={<div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:10,color:t.textMuted}}>Month:</span><Sel value={teamMonth} onChange={setTeamMonth} options={MS} label="All Months" t={t}/></div>}><div style={{overflowX:"auto",borderRadius:12,border:"1px solid "+t.cardBorder,background:t.card}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12.5}}><thead><tr>{THDSL.map((h,i)=><th key={i} style={{padding:"11px 14px",textAlign:i===0?"left":"right",color:h.includes("GP")?t.primary:t.textMuted,fontWeight:700,fontSize:10.5,textTransform:"uppercase",letterSpacing:.5,borderBottom:"2px solid "+t.divider,background:h.includes("GP")?t.primaryLight:t.tableBg,whiteSpace:"nowrap",minWidth:i===0?80:100}}>{h}</th>)}</tr></thead><tbody>{sellerSummary.map((r,i)=><tr key={i} onMouseEnter={e=>e.currentTarget.style.background=t.tableHover} onMouseLeave={e=>e.currentTarget.style.background="transparent"} style={{transition:"background .1s"}}><td style={{padding:"11px 14px",fontWeight:700,borderBottom:"1px solid "+t.divider}}>{r.sl}</td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider,background:t.primaryGhost}}><APG actual={r.ga} plan={r.gp} t={t}/></td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider}}><APG actual={r.ra} plan={r.rp} t={t}/></td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider}}><APG actual={r.aa} plan={r.ap} t={t} reverse/></td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider}}><APG actual={r.ua} plan={r.up} t={t} isMoney={false}/></td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider,fontWeight:600,color:mC(r.margin,t)}}>{r.margin.toFixed(2)}%</td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider,fontWeight:600}}>{r.cnt}</td></tr>)}</tbody></table><div style={{padding:"8px 14px",fontSize:10.5,color:t.textMuted,borderTop:"1px solid "+t.divider}}>Each cell: <strong style={{color:t.text}}>Actual</strong> / Plan / <span style={{color:t.green}}>Gap</span> · Ads: lower = better</div></div></Sec>
    {asinData.length>0&&<Sec title="ASIN Detail by Seller (A / P / Gap)" icon="📋" t={t}><div style={{overflowX:"auto",maxHeight:520,overflowY:"auto",borderRadius:12,border:"1px solid "+t.cardBorder,background:t.card}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12.5}}><thead style={{position:"sticky",top:0,zIndex:2}}><tr>{THDASIN.map((h,i)=><th key={i} style={{padding:"11px 14px",textAlign:i<=2?"left":"right",color:h.includes("GP")?t.primary:t.textMuted,fontWeight:700,fontSize:10.5,textTransform:"uppercase",letterSpacing:.5,borderBottom:"2px solid "+t.divider,background:h.includes("GP")?t.primaryLight:t.tableBg,whiteSpace:"nowrap",minWidth:i<=2?70:100}}>{h}</th>)}</tr></thead><tbody>{asinData.map((r,i)=><tr key={i} onMouseEnter={e=>e.currentTarget.style.background=t.tableHover} onMouseLeave={e=>e.currentTarget.style.background="transparent"} style={sellerBreaks.has(i)?{borderTop:"2px solid "+t.divider}:{transition:"background .1s"}}><td style={{padding:"11px 14px",fontFamily:"monospace",fontSize:10.5,borderBottom:"1px solid "+t.divider,color:t.textSec}}>{r.a}</td><td style={{padding:"11px 14px",fontWeight:600,borderBottom:"1px solid "+t.divider,fontSize:11.5,color:t.primary}}>{r.sl||"—"}</td><td style={{padding:"11px 14px",fontWeight:700,borderBottom:"1px solid "+t.divider}}>{r.br}</td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider,background:t.primaryGhost}}><APG actual={r.ga} plan={r.gp} t={t}/></td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider}}><APG actual={r.ra} plan={r.rp} t={t}/></td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider}}><APG actual={r.aa} plan={r.ap} t={t} reverse/></td><td style={{padding:"11px 14px",textAlign:"right",borderBottom:"1px solid "+t.divider}}><APG actual={r.ua} plan={r.up} t={t} isMoney={false}/></td></tr>)}</tbody></table><div style={{padding:"8px 14px",fontSize:10.5,color:t.textMuted,borderTop:"1px solid "+t.divider,position:"sticky",bottom:0,background:t.card}}>{asinData.length} ASINs · Grouped by seller · Ads: lower = better</div></div></Sec>}
...
export default function App(){
      ...
      try{
        ...
        const abk=[...allAsins].map(asin=>{
          ...
          allMonths.forEach(mn=>{
            ...
            mData[mn]={ra:am.rv||0,rp:pm.rv||0,ga:am.gp||0,gp:pm.gp||0,na:am.np||0,np:pm.np||0,aa:am.ad||0,ap:pm.ad||0,ua:am.un||0,up:pm.un||0,sa:am.se||0,sp:pm.se||0,ia:am.im||0,ip:pm.im||0,cra:Math.round((am.cr||0)*10000)/100,crp:planCrM,cta:Math.round((am.ct||0)*10000)/100,ctp:planCtM};
          });
          return{a:asin,br:pd.brand||ad.br||"",sl:ad.sl||"",ga:ad.ga||0,gp:pTot.gp,na:ad.na||0,np:pTot.np,ra:ad.ra||0,rp:pTot.rv,aa:ad.aa||0,ap:pTot.ad,ua:ad.ua||0,up:pTot.un,sa:ad.sa||0,sp:pTot.se,ia:ad.ia||0,ip:pTot.im,cra:Math.round((ad.cra||0)*10000)/100,crp:crP,cta:Math.round((ad.cta||0)*10000)/100,ctp:0,mData};
  ...
  return<div style={{display:"flex",flexDirection:mob?"column":"row",height:"100vh",background:t.bg,fontFamily:"'DM Sans',system-ui,-apple-system,sans-serif",color:t.text,overflow:"hidden",transition:"background .3s"}}>
    ...
    {!mob&&<div style={{width:(tab||!sb)?56:220,background:t.sidebar,borderRight:"1px solid "+t.sidebarBorder,display:"flex",flexDirection:"column",transition:"width .2s",flexShrink:0,overflow:"hidden"}}>
      ...
      <div style={{flex:1,padding:6,overflowY:"auto"}}>{NAV.map(n=><button key={n.id} onClick={()=>setPg(n.id)} style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:(!tab&&sb)?"11px 14px":"11px 0",borderRadius:10,border:"none",cursor:"pointer",marginBottom:3,background:pg===n.id?t.sidebarActive:"transparent",color:pg===n.id?t.primary:t.textSec,justifyContent:(!tab&&sb)?"flex-start":"center",fontSize:12.5,transition:"all .15s ease"}} onMouseEnter={e=>{if(pg!==n.id)e.currentTarget.style.background=t.tableHover}} onMouseLeave={e=>{if(pg!==n.id)e.currentTarget.style.background="transparent"}}><span style={{fontSize:16,flexShrink:0}}>{n.i}</span>{!tab&&sb&&<span style={{fontWeight:pg===n.id?700:500,whiteSpace:"nowrap",letterSpacing:-.1}}>{n.l}</span>}</button>)}</div>
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",paddingBottom:mob?56:0}}>
      ...
      <div style={{background:t.topbar,borderBottom:"1px solid "+t.cardBorder,padding:mob?"10px 14px":"14px 24px",display:"flex",flexDirection:"column",gap:10,flexShrink:0}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>{mob&&<button onClick={()=>setMobileFilters(!mobileFilters)} style={{background:t.primaryLight,border:"1px solid "+t.primary+"33",borderRadius:10,padding:"7px 12px",cursor:"pointer",fontSize:12,color:t.primary,fontWeight:700}}>☰</button>}<span style={{fontSize:mob?15:18,fontWeight:800,color:t.text,letterSpacing:-.3}}>{cn?.i} {cn?.l}</span></div>
      ...
      <div style={{flex:1,overflow:"auto",padding:mob?12:20}}>
        ...
        {pg==="team"&&<TeamPage t={t} fSeller={fSeller} fDaily={fDaily} asinPlanBkData={asinPlanBkState}/>}
